...
const TAB_KEYCODE = 9;
const mapApiKey="AIzaSyDze972Ns9Ooh1XsRAP-1ngmRbB68vBdNc";
...
//      load: {
//        key: 'AIzaSyDze972Ns9Ooh1XsRAP-1ngmRbB68vBdNc'
//      },
...
//   this.filterData('refresh');
   ...
   jQuery('.filter-item .custom-select').each(function(){
        ...
        townOrCity(txt){
            switch(txt){
                case 'ag_cities':return 'Town';break;
              case 'ag_county':return 'County';break;
              case 'ag_regions':return 'Region';break;
          ...
          townOrCitySlug(slug,taxonomy){
            ...
            switch(taxonomy){
                case 'ag_cities':return $link+'/town/'+slug;break;
             case 'ag_regions':return $link+'/'+slug;break;
                  case 'ag_county':return $link+'/'+slug;break;
...
//           var scriptTag='<script src="https://maps.googleapis.com/maps/api/js?key=AIzaSyDze972Ns9Ooh1XsRAP-1ngmRbB68vBdNc&libraries=geometry" ></script>';
...
if(long && lat){
      ...
      this.center.lng=parseFloat(long);
       ...
       fetch("https://maps.googleapis.com/maps/api/geocode/json?address="+this.placeName+",uk&key=AIzaSyDze972Ns9Ooh1XsRAP-1ngmRbB68vBdNc", {
                headers
...
//         {"name": "Deselect All", "term_id": -2},
        {"name": "Personal Care", "term_id": 13271},
//             {"name": "Day Care", "term_id": 112321},
//                  {"name": "Live-in Care", "term_id": 112319},
                    {"name": "Dementia", "term_id": 8335},
                     {"name": "Physical Disability", "term_id": 12145},
                    {"name": "Mental Health", "term_id": 8339},
...
//  function updateRadiusPosition(data,cityCenterLong,cityCenterLat){
//    data.forEach(function (item, index) {
//        
//    
//       
//      item.dynamic_distance_miles=calculateDistance(item.latitude,item.longitude,cityCenterLat,cityCenterLong);
...
//
//        }; 
            fetch("https://maps.googleapis.com/maps/api/geocode/json?address="+item.label+"&key=AIzaSyDze972Ns9Ooh1XsRAP-1ngmRbB68vBdNc", {
                headers
...
//            var tempFilteredData=updateRadiusPosition(app.data.filteredData,cityCenterLong,cityCenterLat);
//     
   console.log('tempFilteredData--->updateRadiusPosition(): ',tempFilteredData);
...
//    console.log('Need to update this location rate to ' +liveRate);
        ...
        addURLParam: function (key, value) {
            ...
            this.redirectParams.push({"key": key, "value": value});
...
//            }).join("&");
        ...
        removeFilter: function (filterSlug) {
            ...
            this.resetPagination();
            this.searchFilters.pop({"slug": filterSlug});
            this.filterData('refresh');
        },
...
//                  this.resetCheckboxes();
  ...
  if (value !== "0") {
                    this.searchFilters.push({"slug": filterType, 'serviceTypeName': optiontxt, 'serviceTypeID': value, "label": label, "value": value, "optionText": optiontxt});
                    }
